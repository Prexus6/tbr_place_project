...
def login_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                return redirect('index')  # nebo 'home', podle názvu vaší URL
    else:
        form = LoginForm()
            ...
            try:
                ...
                if 'secret_answer' in request.POST:
                    if user.secret_answer == form.cleaned_data['secret_answer']:
                        ...
                        messages.info(request, 'If you do not know the answer, please contact the site administrator or register again.')
